...
function LoginContent() {
  ...
  // Show error from callback (e.g. confirmation_failed)
  ...
  async function handleGoogleLogin() {
    ...
    if (error) {
      toast.error("שגיאה בכניסה עם Google: " + error.message)
        ...
        <div className="text-center mb-8">
          <div className="flex items-center justify-center gap-2.5 mb-3">
            <div className="flex h-11 w-11 items-center justify-center rounded-full"
              style={{ backgroundColor: "rgba(255,226,111,0.18)", border: "1.5px solid rgba(255,226,111,0.5)" }}>
              <ShieldCheck className="h-5 w-5" style={{ color: "#ffe26f" }} />
            </div>
            <span className="text-2xl font-bold text-slate-800">ביטחון בשוק ההון</span>
          </div>
          <p className="text-slate-500 text-sm">כניסה לחשבון</p>
          {callbackError === "confirmation_failed" && (
            <p className="mt-2 text-sm text-rose-600 bg-rose-50 rounded-lg px-3 py-2">
              לינק האישור פג תוקף — נסה שוב.
            </p>
          )}
        </div>
          ...
          <CardContent className="space-y-4">
            ...
            <div className="relative">
              ...
              <div className="relative flex justify-center text-xs text-slate-400 bg-white px-3">
                <span className="bg-white px-2">או עם אימייל וסיסמה</span>
              </div>
            </div>
